...
LOAD_GDT:                                       ;Заполнить глобальную таблицу дескрипторов            
    ...
    shl ax,4
    shr dx,4
    mov si,ax  ; младшая часть
    mov di,dx  ; старшая часть
WRITE_GDT:                                      ;Заполнить дескриптор GDT
    lea bx,GDT_GDT
    mov ax,si  ; младшая часть
    mov dx,di  ; старшая часть
    add ax,offset GDT
...
OVERLOAD_CS:                                    ;Перезагрузить сегмент кода на его дескриптор
    db  0eah                                    ;Искуственно сконструированная команда дальнего перехода (far jump)
...
EXC_HANDLER proc near                           ;Процедура вывода обработки исключений
    ...
    pop eax                                     ;И EFLAGS
    sti                                         ;Обязательно, без этого обработка аппаратных прерываний отключена
    db 66H  ; IMPORTANT: префикс размера операнда. Так как мы в 32 битном режиме, то размер операнда стал 16 бит, а адреса все еще 32 бита. db 67H менял бы размер адреса
    retf                                        ;Переход в 16-битный сегмент кода    
EXC_HANDLER     ENDP
...
CLRSCR  proc near                               ;Процедура очистки консоли
    ...
    mov  ax,700h ; листаем окно до конца вниз
    rep  stosw
    popad
    pop  es
    ret
